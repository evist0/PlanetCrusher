	...
	float3 VolumeFog(float3 sourceImg, float dist, float3 ray, float3 rayStart, float3 WorldPosition, float2 texcoord, float depth)
	{
		...
		if (lightControlA.x != 0) {
			...
			for (int i = 0; i < steps; ++i)
			{
				...
				pos += (step + step * rand * 0.8 * volumeSamplingControl.w);
			}
		}
				...
				//LIGHT 1
					...
					if (_visibleLightsCount > 1) {//v1.9.9.5
						if (controlByColor == 0 || (controlByColor == 1 &&
							...
							sourceImg = sourceImg + lightControlA.z * o * 0.04*sourceImg * light.color * light.distanceAttenuation * light.shadowAttenuation / powDIVIDE;//*15
						}
					}
																																								 //}
				//LIGHT 2
...
//#endif		
	...
	//	float depth =  Linear01Depth(zsample * (zsample < 1.0)); //////// CHANGE 2 URP
		...
		if (doHeightA == 1) {
			g +=  ComputeHalfSpace(wpos - _WorldSpaceCameraPos.y); //v1.9.9.5 - Ethereal v1.1.8
